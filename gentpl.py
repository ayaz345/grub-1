...
if test x$(TARGET_APPLE_LINKER) = x1; then \
  ...
  $(TARGET_OBJCOPY) $("""
        + cname(defn)
        + """_OBJCOPYFLAGS) --strip-unneeded -R .note -R .comment -R .note.gnu.build-id -R .MIPS.abiflags -R .reginfo -R .rel.dyn -R .note.gnu.gold-version -R .note.gnu.property -R .ARM.exidx $< $@; \
  $(TARGET_OBJCOPY) $("""
        + cname(defn)
        + """_OBJCOPYFLAGS) --strip-unneeded -R .note -R .comment -R .note.gnu.build-id -R .MIPS.abiflags -R .reginfo -R .rel.dyn -R .note.gnu.gold-version -R .note.gnu.property -R .ARM.exidx $< $@; \
fi
...
def manpage(defn, adddeps):
    ...
    rule(
        f"{name}.{mansection}",
        f"{name} {adddeps}",
        f"cat $(top_srcdir)/util/{name}.{mansection} | $(top_builddir)/config.status --file=$@:-",
    )
